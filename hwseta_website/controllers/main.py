...
class Website(openerp.addons.website.controllers.main.Website):
	...
	def confirm_sdf(self, **post):
		...
		if post:
			...
			if post['txtIdDocument']:
				doc_id = registry.get('ir.attachment').create(cr, SUPERUSER_ID,
															  {'name': post.get('txtIdDocument', '').filename,
															   'datas': base64.encodestring(
																   post['txtIdDocument'].read()),
															   'datas_fname': post.get('txtIdDocument', '').filename, })
			...
			for i in range(0, int(optionCount)):
				...
				if tempsdl:
					employer_ids = res_partner_obj.search(cr, SUPERUSER_ID, [('employer_sdl_no', '=', tempsdl)],
														  context=context)
					...
					for lines in employer_obj:
						...
						val = {
							...
							'confirm_sdf_appointment_letter_from_employer': registry.get('ir.attachment').create(cr,
																												 SUPERUSER_ID,
																												 {
																													 'name': post.get(
																														 sdf_appointment_letter,
																														 False).filename,
																													 'datas': base64.encodestring(
																														 post.get(
																															 sdf_appointment_letter).read()),
																													 'datas_fname': post.get(
																														 sdf_appointment_letter,
																														 False).filename, }),
	...
	def get_aacreditationDetails(self, **post):
		...
		cr.execute(
			"select id, name,saqa_qual_id,m_credits from provider_qualification where seta_branch_id = 1 order by id desc limit 10 ")
		...
		cr.execute(
			"select id, name,code from etqe_learning_programme where seta_branch_id = 1 order by id desc limit 10")
		...
		ir_sequence_obj = registry.get('ir.sequence')
		value.update(
			{'provider_accreditation_ref': ir_sequence_obj.get(cr, SUPERUSER_ID, 'provider.accreditation.reference')})
	...
	def confirm_accreditation(self, **post):
		...
		if post:
			...
			if post['CKDocumentUpload']:
				values['ck_documents'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
					'name': post.get('CKDocumentUpload', '').filename,
					'datas': base64.encodestring(post['CKDocumentUpload'].read()),
					'datas_fname': post.get('CKDocumentUpload', '').filename, })
			if post['OrganogramUpload']:
				values['organogram_documents'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
					'name': post.get('OrganogramUpload', '').filename,
					'datas': base64.encodestring(post['OrganogramUpload'].read()),
					'datas_fname': post.get('OrganogramUpload', '').filename, })

			if post['ciproDocument']:
				values['cipro_documents'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
					'name': post.get('ciproDocument', '').filename,
					'datas': base64.encodestring(post['ciproDocument'].read()),
					'datas_fname': post.get('ciproDocument', '').filename, })
			if post['taxDocument']:
				values['tax_clearance'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
					'name': post.get('taxDocument', '').filename,
					'datas': base64.encodestring(post['taxDocument'].read()),
					'datas_fname': post.get('taxDocument', '').filename, })
			if post['cvDocument']:
				values['director_cv'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
					'name': post.get('cvDocument', '').filename,
					'datas': base64.encodestring(post['cvDocument'].read()),
					'datas_fname': post.get('cvDocument', '').filename, })
			if post['qualificationDocument']:
				values['certified_copies_of_qualifications'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
					'name': post.get('qualificationDocument', '').filename,
					'datas': base64.encodestring(post['qualificationDocument'].read()),
					'datas_fname': post.get('qualificationDocument', '').filename, })

			if post['lease_agreement']:
				values['lease_agreement_document'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
					'name': post.get('lease_agreement', '').filename,
					'datas': base64.encodestring(post['lease_agreement'].read()),
					'datas_fname': post.get('lease_agreement', '').filename, })

			if post['professional_body_registration']:
				values['professional_body_registration'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
					'name': post.get('professional_body_registration', '').filename,
					'datas': base64.encodestring(post['professional_body_registration'].read()),
					'datas_fname': post.get('professional_body_registration', '').filename, })
			if post['workplace_agreement']:
				values['workplace_agreement'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
					'name': post.get('workplace_agreement', '').filename,
					'datas': base64.encodestring(post['workplace_agreement'].read()),
					'datas_fname': post.get('workplace_agreement', '').filename, })
			if post['business_residence_proof']:
				values['business_residence_proof'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
					'name': post.get('business_residence_proof', '').filename,
					'datas': base64.encodestring(post['business_residence_proof'].read()),
					'datas_fname': post.get('business_residence_proof', '').filename, })
			if post['provider_learning_material']:
				values['provider_learning_material'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
					'name': post.get('provider_learning_material', '').filename,
					'datas': base64.encodestring(post['provider_learning_material'].read()),
					'datas_fname': post.get('provider_learning_material', '').filename, })
			if post['skills_programme_registration_letter']:
				values['skills_programme_registration_letter'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID,
																									  {'name': post.get(
																										  'skills_programme_registration_letter',
																										  '').filename,
																									   'datas': base64.encodestring(
																										   post[
																											   'skills_programme_registration_letter'].read()),
																									   'datas_fname': post.get(
																										   'skills_programme_registration_letter',
																										   '').filename, })
			if post['company_profile_and_organogram']:
				values['company_profile_and_organogram'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
					'name': post.get('company_profile_and_organogram', '').filename,
					'datas': base64.encodestring(post['company_profile_and_organogram'].read()),
					'datas_fname': post.get('company_profile_and_organogram', '').filename, })
			if post['quality_management_system']:
				values['quality_management_system'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
					'name': post.get('quality_management_system', '').filename,
					'datas': base64.encodestring(post['quality_management_system'].read()),
					'datas_fname': post.get('quality_management_system', '').filename, })
			...
			if post:
				...
				while i <= post_len:
					...
					if post.get(campus_assessor):
						...
						campus_assessor_obj = hr_employee_obj.browse(cr, SUPERUSER_ID,
																	 [int(ids) for ids in campus_assessor if ids])
						for hr in campus_assessor_obj:
							...
							campus_assessor_vals = {
								...
								'campus_assessor_sla_document': registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
									'name': post.get(campus_assessor_sla, False).filename,
									'datas': base64.encodestring(post.get(campus_assessor_sla, False).read()),
									'datas_fname': post.get(campus_assessor_sla, False).filename, })
							}
					...
					if post.get(campus_moderator):
						...
						campus_moderator_obj = hr_employee_obj.browse(cr, SUPERUSER_ID,
																	  [int(ids) for ids in campus_moderator if ids])
						for moderator_hr in campus_moderator_obj:
							...
							campus_moderator_vals = {
								...
								'campus_moderator_sla_document': registry.get('ir.attachment').create(cr, SUPERUSER_ID,
																									  {'name': post.get(
																										  campus_moderator_sla,
																										  False).filename,
																									   'datas': base64.encodestring(
																										   post.get(
																											   campus_moderator_sla,
					...
					if post.get(campus_qual):
						...
						idx = 0
						for q_lines in provider_qualification_obj.browse(cr, SUPERUSER_ID, [int(key) for key, value in
																							qualification_id.iteritems()]):
							...
							if campus_assessor_vals:
								q_vals.update({
									...
									'assessor_sla_document': campus_assessors_list[idx][2][
										'campus_assessor_sla_document'],
								})
							if campus_moderator_vals:
								q_vals.update({
									'moderators_id': campus_moderators_list[idx][2]['moderators_id'],
									'moderator_sla_document': campus_moderators_list[idx][2][
										'campus_moderator_sla_document'],
								})
					...
					if post.get(campus_skill):
						...
						for s_lines in skill_obj.browse(cr, SUPERUSER_ID,
														[int(key) for key, value in skill_id.iteritems()]):
							campus_skill_line = []
					...
					if post.get(campus_learning):
						...
						for l_lines in learning_obj.browse(cr, SUPERUSER_ID,
														   [int(key) for key, value in learning_id.iteritems()]):
			...
			if post.get('assesor'):
				...
				for hr in assessor_obj:
					...
					assessor_vals = {
						...
						'awork_email': hr.work_email,
						'awork_phone': hr.person_cell_phone_number,
						'assessor_sla_document': registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
							'name': post.get(assessor_sla_dcument_key).filename,
							'datas': base64.encodestring(post.get(assessor_sla_dcument_key).read()),
							'datas_fname': post.get(assessor_sla_dcument_key).filename, }),
						'assessor_notification_letter': registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
							'name': post.get(assessor_notification_key).filename,
							'datas': base64.encodestring(post.get(assessor_notification_key).read()),
							'datas_fname': post.get(assessor_notification_key).filename, })
					}
			...
			if post.get('moderator'):
				...
				for hr_moderator in moderator_obj:
					...
					moderator_vals = {
						...
						'moderator_sla_document': registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
							'name': post.get(moderator_sla_document_key).filename,
							'datas': base64.encodestring(post.get(moderator_sla_document_key).read()),
							'datas_fname': post.get(moderator_sla_document_key).filename, }),
						'moderator_notification_letter': registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
							'name': post.get(moderator_notification_key).filename,
							'datas': base64.encodestring(post.get(moderator_notification_key).read()),
							'datas_fname': post.get(moderator_notification_key).filename, })
			...
			provider_accreditation_obj.browse(cr, SUPERUSER_ID, res).write({'qualification_ids': q_vals_line,
																			...
																			'provider_accreditation_ref': post.get(
	...
	def get_assessorModeratorDetails(self, **post):
		...
		cr.execute(
			"select id, name,saqa_qual_id from provider_qualification where seta_branch_id = 1 or is_ass_mod_linked = True order by id desc limit 100 ")
		qualification_data = cr.fetchall()
		value.update({'qualification': qualification_data})
	...
	def confirm_assessor_moderator(self, **post):
		...
		if post:
			...
			if post.get('radioAssMod', False) and post.get('radioAssMod') == 'already_registered' and post.get(
					'ex_ass_mod') == 'ex_ass':
			...
			if post.get('radioAssMod', False) and post.get('radioAssMod') == 'assessor':
				...
				if post['txtIdDocument']:
					values['id_document'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
						'name': post.get('txtIdDocument', '').filename,
						'datas': base64.encodestring(post['txtIdDocument'].read()),
						'datas_fname': post.get('txtIdDocument', '').filename, })
				if post['registrationDoc']:
					values['registrationdoc'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
						'name': post.get('registrationDoc', '').filename,
						'datas': base64.encodestring(post['registrationDoc'].read()),
						'datas_fname': post.get('registrationDoc', '').filename, })
				if post['professionalbodyDoc']:
					values['professionalbodydoc'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
						'name': post.get('professionalbodyDoc', '').filename,
						'datas': base64.encodestring(post['professionalbodyDoc'].read()),
						'datas_fname': post.get('professionalbodyDoc', '').filename, })
				if post['SRAM_Doc']:
					values['sram_doc'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
						'name': post.get('SRAM_Doc', '').filename,
						'datas': base64.encodestring(post['SRAM_Doc'].read()),
						'datas_fname': post.get('SRAM_Doc', '').filename, })
				if post.get('type_document', False):
					values['unknown_type_document'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
						'name': post.get('type_document', '').filename,
						'datas': base64.encodestring(post['type_document'].read()),
						'datas_fname': post.get('type_document', '').filename, })
				if post['cv_document']:
					values['cv_document'] = registry.get('ir.attachment').create(cr, SUPERUSER_ID, {
						'name': post.get('cv_document', '').filename,
						'datas': base64.encodestring(post['cv_document'].read()),
						'datas_fname': post.get('cv_document', '').filename, })
			...
			if post.get('qualification_idss'):
				...
				for key, value in qualification_id.iteritems():
					...
					for val in value:
						qualification_line_list.append(val)
				for q_lines in qualification_obj.browse(cr, SUPERUSER_ID,
														[int(key) for key, value in qualification_id.iteritems()]):
			...
			assessors_moderators_register_obj.write(cr, SUPERUSER_ID, res, {'qualification_ids': q_vals_line,
																			'assessors_moderators_ref': post.get(
																				'assessors_moderators_ref', '')})
	...
	def validate_assesor_number(self, search='', **post):
		...
		assessors_id = assessor_obj.search(cr, SUPERUSER_ID, [('assessor_seq_no', '=', str(post['assesor_number']))],
										   context=context)
		if assessors_id:
					...
					if qualifcation.qual_unit_type == 'qual':
						for asssessor_qualifcation in qualifcation:
							for lines in asssessor_qualifcation.qualification_line_hr:
								for quali in provider_obj.browse(cr, SUPERUSER_ID,
																 asssessor_qualifcation.qualification_hr_id.id):
									for q_line in quali.qualification_line:
										if q_line.id_no == lines.id_no:
											main_quali_line.append(q_line.id)
						main_quali = {
							asssessor_qualifcation.qualification_hr_id.id: (
							asssessor_qualifcation.qualification_hr_id.name, main_quali_line),
						}
	...
	def get_locality(self, **post):
		...
		result = []
		if post.get('suburb'):
			cr.execute("select city_id,province_id,country_id,postal_code from res_suburb where id=%s" % (
				int(str(post.get('suburb')))))
			suburb = cr.fetchone()
	...
	def check_sdl_number(self, **post):
		...
		result = []
		cr.execute(
			"select id,name,employer_trading_name,employer_sdl_no from res_partner where employer=True  and employer_sdl_no='%s'" % (
				str(post.get('sdl_no'))))
		partner = cr.fetchall()
		if len(partner) == 0:
			return json.dumps(result)
	...
	def get_city(self, **post):
		...
		if post.get('province'):
			...
			cr.execute("select name,urban_rural from res_city where active = True and province_id=%s" % (
				int(str(post.get('province')))))
			...
			for rec in sorted_cities:
				cr.execute(
					"select id,name,urban_rural from res_city where active = True and name = '%s' and urban_rural = '%s'" % (
					(rec[0]), (rec[1])))
				city = cr.fetchone()
				if city:
					cities.append(city)
	...
	def get_suburb(self, **post):
		...
		if post.get('city'):
			...
			for rec in sorted_suburbs:
				cr.execute("select id,name,urban_rural from res_suburb where name = '%s' and urban_rural = '%s'" % (
				(rec[0]), (rec[1])))
				suburb = cr.fetchone()
				if suburb:
	...
	def check_identification_no(self, **post):
		...
		if post.get('identification_no'):
			cr.execute("select id,state,sdf_reference_no from sdf_register where identification_id='%s'" % str(
				post.get('identification_no')))
			sdf = cr.fetchall()
			if len(sdf) == 0:
				cr.execute("select id from hr_employee where is_sdf=true and identification_id='%s'" % str(
					post.get('identification_no')))
	...
	def check_identification_no_assessor(self, **post):
		...
		if post.get('identification_no'):
			cr.execute(
				"select id from assessors_moderators_register where final_state != 'Rejected' and identification_id='%s'" % str(
					post.get('identification_no')))
			assessor = cr.fetchall()
			if len(assessor) == 0:
				cr.execute(
					"select id from hr_employee where is_assessors=true and assessor_moderator_identification_id='%s'" % str(
						post.get('identification_no')))
	...
	def check_email_id(self, **post):
		...
		if post.get('email'):
			...
			sdf = cr.fetchall()
			cr.execute(
				"select id from assessors_moderators_register where work_email='%s' and final_state != 'Rejected'" % str(
					post.get('email')).strip())
			assessor = cr.fetchall()
			cr.execute("select id from provider_accreditation where email='%s' and final_state != 'Rejected'" % str(
				post.get('email')).strip())
	...
	def check_sdl_numbers(self, **post):
		...
		if post.get('sdl_number'):
			cr.execute("""select id from provider_accreditation where provider_accreditation."txtSDLNo"='%s'""" % str(
				post.get('sdl_number')))
			provider = cr.fetchall()
			if len(provider) == 0:
				value.update({'result': 0})
	...
	def check_vat_registartion_no(self, **post):
		...
		if post.get('vat_registration_no'):
			cr.execute(
				"""select id from provider_accreditation where provider_accreditation."txtVATRegNo"='%s'""" % str(
					post.get('vat_registration_no')))
			provider = cr.fetchall()
			if len(provider) == 0:
				value.update({'result': 0})
				cr.execute(
	...
	def get_assessor_moderator_number(self, **post):
		...
		if str(post.get('assessor_moderator')) == 'already_registered' and str(
				post.get('ex_assessor_ex_moderator')) == 'ex_ass':
			value.update({'assessor_moderator': registry['ir.sequence'].get(cr, SUPERUSER_ID, 'assessors.register')})
		if str(post.get('assessor_moderator')) == 'already_registered' and str(
				post.get('ex_assessor_ex_moderator')) == 'ex_mod':
			value.update({'assessor_moderator': registry['ir.sequence'].get(cr, SUPERUSER_ID, 'moderators.register')})
		result.append(value)
		return json.dumps(result)
	...
	def load_more_qualification(self, **post):
		...
		value = {}
		cr.execute(
			"select id, name,saqa_qual_id,m_credits from provider_qualification where id<%s and seta_branch_id = 1 order by id desc limit 100" % (
				int(str(post.get('last_qualification_id')))))
		qualification_data = cr.fetchall()
		for qualification in qualification_data:
			value = {'id': qualification[0], 'name': qualification[1], 'saqa_qual_id': qualification[2],
					 'm_credits': qualification[3]}
			result.append(value)
		return json.dumps(result)
	...
	def load_more_skill(self, **post):
		...
		value = {}
		cr.execute(
			"select id, name,code from skills_programme where id<%s and seta_branch_id = 1 order by id desc limit 100" % (
				int(str(post.get('last_skill_id')))))
		skill_data = cr.fetchall()
	...
	def load_more_lp(self, **post):
		...
		value = {}
		cr.execute(
			"select id, name,code from etqe_learning_programme where id<%s and seta_branch_id = 1 order by id desc limit 100" % (
				int(str(post.get('last_lp_id')))))
		lp_data = cr.fetchall()
		for lp in lp_data:
	...
	def load_more_assessor(self, **post):
		...
		value = {}
		cr.execute(
			"select id, person_name from hr_employee where id<%s and is_assessors=True order by id desc limit 100" % (
				int(str(post.get('assessor_id')))))
		assessor_data = cr.fetchall()
	...
	def get_organisation_name(self, **post):
		...
		result = []
		value = {}
		cr.execute("select id,name from res_partner where employer_sdl_no='%s' and employer=True" % (
			str(post.get('organisation_sdl_no'))))
	...
	def check_assessor_number(self, **post):
		...
		for skill_id in selected_skill_ids:
			if skill_id:
				cr.execute("select saqa_qual_id from skills_programme where seta_branch_id = 1 and id = '%s'",
						   ([skill_id]))
				skill_qual_id = cr.fetchone()
				if skill_qual_id:
					cr.execute("select id from provider_qualification where seta_branch_id = 1 and saqa_qual_id = %s",
							   ([skill_qual_id[0]]))
					qual_id = cr.fetchone()
					if qual_id:
		...
		for lp_id in selected_lp_ids:
			if lp_id:
				cr.execute("select saqa_qual_id from etqe_learning_programme where seta_branch_id = 1 and id = '%s'",
						   ([lp_id]))
				...
				if lp_qual_id:
					...
					if qual_id:
						selected_qualification_ids.append(qual_id[0])
		assessor_obj = registry.get('hr.employee')
		assessors_id = assessor_obj.search(cr, SUPERUSER_ID, [('is_active_assessor', '=', True),
															  ('assessor_seq_no', '=', str(post.get('assessor_no')))],
										   context=context)
		cr.execute(
			"select id,assessor_seq_no,person_name from hr_employee where is_assessors=True  and assessor_seq_no='%s'" % (
				str(post.get('assessor_no'))))
		assessor = cr.fetchall()
		if len(assessor) == 0:
			return json.dumps(result)
		else:
	...
	def check_moderator_number(self, **post):
		...
		moderator_id = moderator_obj.search(cr, SUPERUSER_ID, [('is_active_moderator', '=', True), (
		'moderator_seq_no', '=', str(post.get('moderator_no')))], context=context)
		cr.execute(
			"select id,moderator_seq_no,person_name from hr_employee where is_moderators=True and moderator_seq_no='%s'" % (
				str(post.get('moderator_no'))))
	...
	def validate_ex_assesor_number(self, search='', **post):
		...
		assessor_obj = registry.get('hr.employee')
		provider_obj = registry.get('provider.qualification')
		assessors_id = assessor_obj.search(cr, SUPERUSER_ID, [('assessor_seq_no', '=', str(post['txtAssNumber']))],
										   context=context)
		if assessors_id:
			data_assesor = assessor_obj.browse(cr, SUPERUSER_ID, assessors_id, context=context)
			for data in data_assesor:
	...
	def validate_ex_moderator_number(self, search='', **post):
		...
		moderator_obj = registry.get('hr.employee')
		provider_obj = registry.get('provider.qualification')
		moderator_id = moderator_obj.search(cr, SUPERUSER_ID, [('moderator_seq_no', '=', str(post['txtModNumber']))],
											context=context)
		if moderator_id:
	...
	def get_accreditation_details(self, search='', **post):
		...
		provider_id = accreditation_obj.search(cr, SUPERUSER_ID,
											   [('provider_accreditation_num', '=', str(post['accreditation_number']))],
											   context=context)
		if provider_id:
					...
					for qualifcation in data.qualification_ids:
						...
						for provider_qualifcation in qualifcation:
							for lines in provider_qualifcation.qualification_line:
								for quali in provider_qualification_obj.browse(cr, SUPERUSER_ID,
																			   provider_qualifcation.qualification_id.id):
									for q_line in quali.qualification_line:
										if q_line.id_no == lines.id_data:
											main_quali_line.append(q_line.id)
						main_quali = {
							provider_qualifcation.qualification_id.id: (
							provider_qualifcation.qualification_id.name, main_quali_line),
						}
					...
					for provider_skills in data.skills_programme_ids:
						...
						for provider_skill in provider_skills:
							for lines in provider_skill.unit_standards_line:
								for p_skill in provider_skill_obj.browse(cr, SUPERUSER_ID,
																		 provider_skill.skills_programme_id.id):
									for s_line in p_skill.unit_standards_line:
										if s_line.id_no == lines.id_no:
											main_skill_line.append(s_line.id)
						skills = {
							provider_skill.skills_programme_id.id: (
							provider_skill.skills_programme_id.name, main_skill_line),
						}
			...
			vals.update(
				{'master_qualifications': main_qualifications, 'master_skills': main_skills, 'assessors': assessor_list,
				 'moderators': moderator_list, 'contacts': contact_list})
			result.append(vals)
		return json.dumps(result)
