...
class sdf_register(models.Model):
	...
	def action_submit_button(self):
		...
		for employer_data in self.employer_ids:
			...
			necessary_person = prov_necessary[province]

			dbg("PROVINCEMAN:"+str(province))
			#user = self.env['res.users'].search([('province_ids','=',province),('related_partner.function','=','Provincial Manager')])
			user = self.env['res.users'].search([('login', '=', necessary_person)])
			for province in user.province_ids:
				dbg("PROVINCEMAN:"+str(province))
			internal_mail_template_id = ir_model_data_obj.get_object_reference('hwseta_sdp', 'email_template_sdf_individual_register_submit_internal')
